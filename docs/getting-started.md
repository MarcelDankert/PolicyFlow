...
6. advance phases only when their prerequisite workflow phases are completed
7. execute `planning`, `architecture-check`, `review`, and `qa` as real workflow phases
8. treat the workflow as the steering artifact for the change, not as retrospective documentation
9. keep same-PR workflow edits limited to same-scope clarifications
